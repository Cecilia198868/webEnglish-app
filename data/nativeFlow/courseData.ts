...
function getNativeFlowSentenceAudioSrc(levelId: NativeFlowLevelId, sentenceId: number) {
...
export function getNativeFlowSentence(levelId: string, sentenceId: number) {
  ...
  const translatedChinese =
    nativeFlowChineseTranslations[level.id]?.[normalizedSentenceId - 1] || "";
  const sentence =
    sourceSentence
      ? {
          ...sourceSentence,
          audioSrc: getNativeFlowSentenceAudioSrc(level.id, sourceSentence.id),
